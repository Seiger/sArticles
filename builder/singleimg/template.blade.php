...
<div class="row form-row">
    <div class="col-auto col-title-7">
        <div id="image_for_{{$id ?? ''}}" class="image_for_field" data-image="{{$value['src'] ?? 'singleimg'}}" onclick="BrowseServer('{{$id ?? ''}}')" style="background-image: url('{{EVO_SITE_URL.($value['src'] ?? '')}}')"></div>
    </div>
    <div class="col">
